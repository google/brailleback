...
public class TableInfo implements Parcelable {
  ...
  /**
   * Returns {@code true} if this is to be considered an 8 dot table, a.aka. computer braille table.
   * If this returns {@code false}, this table is a literary braille table, where prefixes and
   * multi-cell characters are commonly used.
   */
  ...
  /**
   * Returns the contraction 'grade' for this table. Lower grades mean less contractions. Depending
   * on language, grade 0 or 1 is the lowest grade.
   */
  public int getGrade() {
    return mGrade;
  }
  ...
  /**
   * Returns the unique identifier for this table, which is used when requesting a table to be used
   * for translation. While the identifiers are typically mnemonic, clients should consider them as
   * internal and opaque. Identifiers are stable over releases and can be stored in user settigns
   * etc, however.
   */
  public String getId() {
    return mId;
  ...
  /**
   * Returns the variant description of this table, which helps the user to distinguish table with
  ...
  private TableInfo(Parcel in) {
    mLocale = new Locale(in.readString(), in.readString(), in.readString());
