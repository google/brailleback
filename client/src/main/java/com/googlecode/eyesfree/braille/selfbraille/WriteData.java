...
/**
 * Represents what should be shown on the braille display for a part of the accessibility node tree.
 */
public class WriteData implements Parcelable {
  ...
  /**
   * Sets the text to be displayed when the accessibility node associated with this instance has
   * focus. If this method is not called (or {@code text} is {@code null}), this client relinquishes
  ...
  /**
   * Sets the start position in the text of a text selection or cursor that should be marked on the
   * display. A negative value (the default) means no selection will be added.
   */
  ...
  /**
   * Sets the end of the text selection to be marked on the display. This value should only be
   * non-negative if the selection start is non-negative. If this value is <= the selection start,
   * the selection is a cursor. Otherwise, the selection covers the range from start(inclusive) to
   * end (exclusive).
   *
  ...
  /**
   * Checks constraints on the fields that must be satisfied before sending this instance to the
   * self braille service.
   *
  ...
  /**
   * {@inheritDoc} <strong>Note:</strong> The {@link AccessibilityNodeInfo} will be recycled by this
   * method, don't try to use this more than once.
   */
  @Override
  ...
  private WriteData(Parcel in) {
    mAccessibilityNodeInfo = AccessibilityNodeInfo.CREATOR.createFromParcel(in);
    mText = in.readString();
