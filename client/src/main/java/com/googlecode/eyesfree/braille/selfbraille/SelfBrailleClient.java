 ...
 *
 * <p>Threading: Instances of this object should be created and shut down in a thread with a {@link
 * Looper} associated with it. Other methods may be called on any thread.
 */
public class SelfBrailleClient {
  ...
  private static final String ACTION_SELF_BRAILLE_SERVICE =
      "com.google.android.accessibility.braille.service.ACTION_SELF_BRAILLE_SERVICE";
  private static final String BRAILLE_BACK_PACKAGE = "com.google.android.accessibility.brailleback";
  private static final Intent mServiceIntent =
      new Intent(ACTION_SELF_BRAILLE_SERVICE).setPackage(BRAILLE_BACK_PACKAGE);
  /**
   * SHA-1 hash value of the Eyes-Free release key certificate, used to sign BrailleBack. It was
   * generated from the keystore with: $ keytool -exportcert -keystore <keystorefile> -alias
   * android.keystore \ > cert $ keytool -printcert -file cert
   */
  ...
  /** Written in handler thread, read in any thread calling methods on the object. */
  private volatile Connection mConnection;
  /** Protected by synchronizing on mHandler. */
  private int mNumFailedBinds = 0;

  /**
   * Constructs an instance of this class. {@code context} is used to bind to the self braille
   * service. The current thread must have a Looper associated with it. If {@code allowDebugService}
   * is true, this instance will connect to a BrailleBack service without requiring it to be signed
  ...
  /**
   * Shuts this instance down, deallocating any global resources it is using. This method must be
  ...
  private void doBindService() {
    ...
    if (!mContext.bindService(mServiceIntent, localConnection, Context.BIND_AUTO_CREATE)) {
  ...
  private boolean verifyPackage() {
    ...
    if (mAllowDebugService) {
      Log.w(
          LOG_TAG,
          String.format(
              "*** %s connected to BrailleBack with invalid (debug?) " + "signature ***",
    ...
    public void onServiceConnected(ComponentName className, IBinder binder) {
      if (!verifyPackage()) {
        Log.w(
            LOG_TAG,
            String.format(
                "Service certificate mismatch " + "for %s, dropping connection",
