...
public class AudioSystemCompatUtils {
  ...
  private static final Method METHOD_isStreamActive =
      CompatUtils.getMethod(CLASS_AudioSystem, "isStreamActive", int.class, int.class);
  private static final Method METHOD_LEGACY_isStreamActive =
      CompatUtils.getMethod(CLASS_AudioSystem, "isStreamActive", int.class);
   ...
   *
   * <p>The inPastMs value is only used by the system on API 11+.
   *
   * @param stream The stream ID to query
   * @param inPastMs The time interval allowance, in milliseconds, for checking active streams
   * @return {@code true} if the stream is or was open in the past {@code inPastMs} milliseconds,
