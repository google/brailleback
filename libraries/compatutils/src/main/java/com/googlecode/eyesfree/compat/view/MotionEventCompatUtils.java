...
public class MotionEventCompatUtils {
  ...
  private static final Method METHOD_getSource =
      CompatUtils.getMethod(CLASS_MotionEvent, "getSource");
  private static final Method METHOD_setSource =
      CompatUtils.getMethod(CLASS_MotionEvent, "setSource", int.class);
  private static final Method METHOD_setDownTime =
      CompatUtils.getMethod(CLASS_MotionEvent, "setDownTime", long.class);
   ...
   *
   * <p>Introduced in API Level 12. This method has no effect when called using earlier API levels.
   *
  ...
  /**
   * Sets the time (in ms) when the user originally pressed down to start a stream of position
   * events.
   *
   * <p>Not a public API. This method has no effect when called using unsupported API levels.
   *
   * @param event The event to modify.
   */
  ...
  /**
   * Converts a hover {@link MotionEvent} to touch event by changing its action and source. Returns
   * an modified clone of the original event.
   *
   * <p>The following types are affected:
   *
   * <ul>
