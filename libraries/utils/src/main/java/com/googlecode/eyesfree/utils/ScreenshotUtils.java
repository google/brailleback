...
public class ScreenshotUtils {
  ...
  /**
   * Returns a screenshot with the contents of the current display that matches the current display
   * rotation.
   *
  ...
  /**
   * Creates a new {@link Bitmap} object with a rectangular region of pixels from the source bitmap.
   *
   ...
   * @return A new bitmap of the cropped area, or {@code null} if the source was {@code null} or the
   *     crop parameters were out of bounds.
   */
   ...
   * @return A file where the Bitmap was stored, or {@code null} if the write operation failed.
   */
  public static File writeBitmap(Context context, Bitmap bitmap, String dir, String filename) {
    ...
    try {
      ...
      final boolean compressSuccess =
          bitmap.compress(CompressFormat.PNG, 0 /* quality, ignored for PNG */, outStream);

      if (compressSuccess) {
        LogUtils.log(
            ScreenshotUtils.class, Log.VERBOSE, "Wrote bitmap to %s.", outFile.getAbsolutePath());
        return outFile;
      } else {
        LogUtils.log(
            ScreenshotUtils.class,
            Log.WARN,
            "Bitmap failed to compress to file %s.",
      ...
      LogUtils.log(
          ScreenshotUtils.class,
          Log.WARN,
          "Could not output bitmap file to %s.",
          outFile.getAbsolutePath());
      return null;
    } finally {
      if (outStream != null) {
