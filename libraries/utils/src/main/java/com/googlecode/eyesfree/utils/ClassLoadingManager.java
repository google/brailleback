...
public class ClassLoadingManager {
  ...
  /** A set of classes not found to be loaded. Used to avoid multiple attempts that will fail. */
  ...
  /**
   * Returns a class by given <code>className</code>. The loading proceeds as follows: </br> 1. Try
   * to load with the current context class loader (it caches loaded classes). </br> 2. If (1) fails
   * try if we have loaded the class before and return it if that is the cases. </br> 3. If (2)
   * failed, try to create a package context and load the class. Note: If the package name is null
   * and an attempt for loading of a package context is required the it is extracted from the class
   * name.
   *
   ...
   */
  public Class<?> loadOrGetCachedClass(
      Context context, CharSequence className, CharSequence packageName) {
    ...
    if (TextUtils.isEmpty(packageName)) {
      ...
      if (lastDotIndex < 0) {
        LogUtils.log(this, Log.DEBUG, "Missing package name. Failed to load class: %s", className);
        return null;
      }
    ...
    try {
      ...
      LogUtils.log(
          this, Log.ERROR, "Error encountered. Failed to load outside class: %s", classNameStr);
    }
   ...
   *
   * <p>If a class cannot be loaded by the default {@link ClassLoader}, this method will attempt to
   * use the loader for the specified app package.
   */
  public boolean checkInstanceOf(
    ...
    final Class<?> referenceClass =
        loadOrGetCachedClass(context, referenceClassName, loaderPackage);
