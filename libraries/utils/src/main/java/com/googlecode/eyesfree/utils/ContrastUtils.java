...
public class ContrastUtils {
  ...
  public static double calculateLuminance(int color) {
    ...
    for (int i = 0; i < sRGB.length; ++i) {
      lumRGB[i] =
          (sRGB[i] <= 0.03928d) ? sRGB[i] / 12.92d : Math.pow(((sRGB[i] + 0.055d) / 1.055d), 2.4d);
    }
  ...
  /**
   * Converts a collection of {@code int} representations of colors to a string representation of
   * those colors in hex format.
   *
