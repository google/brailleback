...
public class AccessibilityNodeInfoUtils {
   ...
   *   <li>The node is a top-level list item (see {@link #isTopLevelScrollItem(Context,
  ...
  public static boolean isAccessibilityFocusable(
    ...
    if ((Build.VERSION.SDK_INT < 16)) {
      ...
      if (isTopLevelScrollItem(context, node)
          && (isSpeakingNode(context, node) || hasNonActionableSpeakingChildren(context, node))) {
        return true;
      }
    }
   ...
   * calling {@link #shouldFocusNode} at each level of the view hierarchy.
   */
      ...
      try {
        ...
        if (!isVisibleOrLegacy(child)) {
          LogUtils.log(
              AccessibilityNodeInfoUtils.class,
              Log.VERBOSE,
              "Child %d is invisible, skipping it",
              i);
          continue;
        }
        ...
        if (FILTER_ACCESSIBILITY_FOCUSABLE.accept(context, child)) {
          LogUtils.log(
              AccessibilityNodeInfoUtils.class,
              Log.VERBOSE,
              "Child %d is focusable, skipping it",
              i);
          continue;
        }
  ...
  /**
   * Determines if the current item is at the edge of a list by checking the scrollable predecessors
   * of the items on either or both sides.
   *
   * @param context The parent context.
   * @param node The node to check.
  ...
  public static boolean isEdgeListItem(
    ...
    if ((direction <= 0)
        && isMatchingEdgeListItem(
            context, node, NodeFocusFinder.SEARCH_BACKWARD, FILTER_SCROLL_BACKWARD.and(filter))) {
      return true;
    }

    if ((direction >= 0)
        && isMatchingEdgeListItem(
            context, node, NodeFocusFinder.SEARCH_FORWARD, FILTER_SCROLL_FORWARD.and(filter))) {
      return true;
    }
  ...
  private static boolean isMatchingEdgeListItem(
      Context context, AccessibilityNodeInfoCompat cursor, int direction, NodeFilter filter) {
   ...
   */
  public static List<AccessibilityNodeInfoCompat> searchAllFromBfs(
      Context context, AccessibilityNodeInfoCompat node, NodeFilter filter) {
    if (node == null) {
      return null;
    }
  ...
  /**
   * Performs in-order traversal from a given node in a particular direction until a node matching
   * the specified filter is reached.
   *
   * @param context The parent context.
   * @param root The root node to traverse from.
   * @param filter The filter to satisfy.
   * @return The first node reached via in-order traversal that satisfies the filter.
   */
  public static AccessibilityNodeInfoCompat searchFromInOrderTraversal(
    ...
    while ((currentNode != null)
        && !seenNodes.contains(currentNode)
  ...
  /**
   * Helper method that returns {@code true} if the specified node is visible to the user or if the
   * current SDK doesn't support checking visibility.
   */
   ...
   * #shouldFocusNode(Context, AccessibilityNodeInfoCompat)}.
   */
  ...
  private static final NodeActionFilter FILTER_SCROLL_FORWARD =
      new NodeActionFilter(AccessibilityNodeInfoCompat.ACTION_SCROLL_FORWARD);

  private static final NodeActionFilter FILTER_SCROLL_BACKWARD =
      new NodeActionFilter(AccessibilityNodeInfoCompat.ACTION_SCROLL_BACKWARD);

  /**
   * Convenience class for a {@link NodeFilter} that checks whether nodes support a specific action
   * or set of actions.
   */
  ...
  /** Compares two AccessibilityNodeInfos in left-to-right and top-to-bottom fashion. */
