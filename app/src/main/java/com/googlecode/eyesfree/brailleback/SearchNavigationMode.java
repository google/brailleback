...
/**
 * Navigation mode that is based on incremental search. Used as a separate navigation mode in the
 * BrailleBackService.
 */
class SearchNavigationMode implements NavigationMode {
  ...
  public void onActivate() {
    ...
    if (!AccessibilityNodeInfoRef.isNull(mInitialNode)) {
      ...
      mInitialNode.get().performAction(AccessibilityNodeInfo.ACTION_ACCESSIBILITY_FOCUS);
    }
  ...
  /**
   * Intended to be called before {@link onActivate} so that an initial node is set and can be
   * re-focused if search mode is escaped.
   */
  ...
  public boolean onPanLeftOverflow(DisplayManager.Content content) {
    ...
    return mFeedbackManager.emitOnFailure(
        nextResult(NodeFocusFinder.SEARCH_BACKWARD), FeedbackManager.TYPE_NAVIGATE_OUT_OF_BOUNDS);
  }
  ...
  public boolean onPanRightOverflow(DisplayManager.Content content) {
    ...
    return mFeedbackManager.emitOnFailure(
        nextResult(NodeFocusFinder.SEARCH_FORWARD), FeedbackManager.TYPE_NAVIGATE_OUT_OF_BOUNDS);
  }
  ...
  /** Syncs accessibility focus back to the node focused when search mode first activated. */
  private void syncBackToInitial() {
    ...
    try {
      ...
      if (!AccessibilityNodeInfoRef.isNull(mInitialNode)) {
        ...
        mFeedbackManager.emitOnFailure(
            focused.performAction(AccessibilityNodeInfoCompat.ACTION_CLEAR_ACCESSIBILITY_FOCUS),
            FeedbackManager.TYPE_COMMAND_FAILED);
      }
    } finally {
      focused.recycle();
  ...
  /** Calls the listener. Leaves accessibility focus where it is (on the result). */
  private void finishSearch() {
    mSearchStateListener.onSearchFinished();
  }

  /** Translates braille dots into a character sequence and appends it to the current query text. */
  private boolean handleBrailleKey(int dots) {
    ...
    if (!TextUtils.isEmpty(text)) {
      return mFeedbackManager.emitOnFailure(
          tryAddQueryText(text), FeedbackManager.TYPE_NAVIGATE_OUT_OF_BOUNDS);
    }
  ...
  /**
   * Evaluates the search with the current query and returns whether a match was found searching
   * forward. Tries to match with last matched item.
   */
   ...
   * Ordering of results taken from linear navigation. Returns whether there is another result in
   * that direction.
   */
    ...
    try {
      while (next.get() != null) {
        if (nodeMatchesQuery(next.get())) {
          ...
          focusableNext =
              AccessibilityNodeInfoUtils.findFocusFromHover(mAccessibilityService, next.get());
      ...
      return focusableNext.performAction(AccessibilityNodeInfo.ACTION_ACCESSIBILITY_FOCUS);
    } finally {
  ...
  /**
   * Will delete the last entered character if it exists. If not, will exit incremental search mode
   * without syncing to the selected result.
   */
  ...
  /**
   * Returns whether the specified node's description matches the current query text (case
   * insensitive).
   */
  private boolean nodeMatchesQuery(AccessibilityNodeInfoCompat node) {
    ...
    if (TextUtils.isEmpty(mQueryText)) {
      return AccessibilityNodeInfoUtils.shouldFocusNode(mAccessibilityService, node);
    }

    if (node == null || mSelfBrailleManager.hasContentForNode(node)) {
      return false;
    }

    CharSequence nodeText = LabelingUtils.getNodeText(node, mLabelManager);
    if (nodeText == null) {
      return false;
    }
  ...
  /**
   * Updates the display to show the current search result, with cursor at the end of the matching
   * text. The text will correspond to the text of matched node, which may not be the text of the
   * focused node.
   */
  private void brailleMatchedOrFocusedNode() {
    if (getCurrentNode() != null) {
      DisplayManager.Content content = formatNodeToBraille(getCurrentNode());
      if (content != null) {
        mDisplayManager.setContent(content);
      }
    }
  ...
  /**
   * Returns the last matching node if available. Otherwise falls back to the currently focused
   * node.
   */
  ...
  /**
   * Formats some braille content from an {@link AccessibilityNodeInfoCompat}. Marks the description
   * with a cursor right after the last character matching the search query. Returns content with
   * keep pan strategy by default, or reset pan strategy if there is no matched node stored yet.
   */
  private DisplayManager.Content formatNodeToBraille(AccessibilityNodeInfoCompat node) {
    ...
    AccessibilityNodeInfoCompat spanNode =
        (AccessibilityNodeInfoCompat) DisplaySpans.getEqualSpan(spanned, node);
    int cursorIndex = -1;
    if (spanNode != null) {
    ...
    // prefix.
    int panStrategy =
        AccessibilityNodeInfoRef.isNull(mMatchedNode)
