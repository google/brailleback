...
class DefaultBrailleRule implements BrailleRule {
  ...
  private boolean matchesAny(
      Context context, AccessibilityNodeInfoCompat node, Class<?>... classes) {
    for (Class<?> clazz : classes) {
      if (AccessibilityNodeInfoUtils.nodeMatchesClassByType(context, node, clazz)) {
        return true;
      }
    }
    return false;
  ...
  private CustomLabelManager getLabelManager() {
    if (Build.VERSION.SDK_INT >= CustomLabelManager.MIN_API_LEVEL) {
      final BrailleBackService service = BrailleBackService.getActiveInstance();
      if (service != null) {
