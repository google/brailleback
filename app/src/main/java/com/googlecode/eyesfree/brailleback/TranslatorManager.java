...
/**
 * Keeps track of the current braille translator as determined by the current user locale and
 * settings.
 *
 * <p>Threading: This class is not thread safe,except {@link getTranslator} and {@link
 * #getUncontractedTranslator} may be called from any thread. Note that the result from calling
 * those functions may not be consistent while the selected tables change. This shouldn't be a
 * problem if a caller uses the callback API to retranslate the display content on configuration
 * changes.
 */
public class TranslatorManager implements SharedPreferences.OnSharedPreferenceChangeListener {
  ...
  /**
   * Callback interface to be invoked when one or both of the current translation tables have
   * changed.
   */
  public interface OnTablesChangedListener {
    /**
     * Called when one or both translation tables have changed, including when the translation
     * service is initialized so that tables are available.
     */
  ...
  public TranslatorManager(final Context context) {
    mPrefBrailleTypeKey = context.getString(R.string.pref_braille_type_key);
    mPrefBrailleTypeSixDotValue = context.getString(R.string.pref_braille_type_six_dot_value);
    mPrefBrailleTypeEightDotValue = context.getString(R.string.pref_braille_type_eight_dot_value);
    mPrefBrailleTypeDefault = context.getString(R.string.pref_braille_type_default);
    mPrefSixDotBrailleTableKey = context.getString(R.string.pref_six_dot_braille_table_key);
    mPrefEightDotBrailleTableKey = context.getString(R.string.pref_eight_dot_braille_table_key);
    mTableValueDefault = context.getString(R.string.table_value_default);
            ...
            new TranslatorClient.OnInitListener() {
              ...
              public void onInit(int status) {
                if (status != TranslatorClient.SUCCESS) {
                  LogUtils.log(
                      TranslatorManager.this, Log.ERROR, "Couldn't initialize braille translator");
                  return;
                }
  ...
  /**
   * Returns a translator to be used when contractions shouldn't be used. This is for use when
   * editing text.
   */
  ...
  public List<TableInfo> getRelatedTables(TableInfo tableInfo) {
    return Collections.unmodifiableList(mLocalesToTables.get(tableInfo.getLocale()));
  }

  /** Reevaluates what should be the current translation table based on the new configuration. */
  ...
  private void updateTranslators() {
    ...
    BrailleTranslator newTranslator = findBrailleTranslator(eightDot, true /*fallback*/);
    if (newTranslator == null) {
      LogUtils.log(this, Log.ERROR, "Couldn't find braille translator " + "for %s", mLocale);
      return;
    }
    ...
    boolean changed =
        !newTranslator.equals(mTranslator)
  ...
  public TableInfo findDefaultTableInfo(boolean eightDot) {
    if (mTables == null) {
  ...
  private BrailleTranslator getTranslatorFromPreferences(boolean eightDot) {
    String key = eightDot ? mPrefEightDotBrailleTableKey : mPrefSixDotBrailleTableKey;
    String value = mSharedPreferences.getString(key, mTableValueDefault);
  ...
  /**
   * Finds a translation table for the {@code userLocale}, falling back on US English if no match is
   * found.
   */
  private BrailleTranslator findBrailleTranslator(boolean eightDot, boolean fallback) {
  ...
  private boolean betterTable(TableInfo first, TableInfo second) {
    ...
    return matchRank(firstLocale, mLocale) > matchRank(secondLocale, mLocale);
  }
  ...
  /** Returns whether eight dot braille is selected in the preferences. */
  private boolean isEightDotBrailleSelected() {
    String value = mSharedPreferences.getString(mPrefBrailleTypeKey, mPrefBrailleTypeDefault);
