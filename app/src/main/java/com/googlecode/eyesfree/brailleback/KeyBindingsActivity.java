...
public class KeyBindingsActivity extends Activity
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    switch (item.getItemId()) {
      case android.R.id.home:
        Intent intent = new Intent(this, BrailleBackPreferencesActivity.class);
  ...
  private void populateListView() {
    ...
    String[] supportedCommands = getResources().getStringArray(R.array.help_supportedCommands);
    String[] descriptions = getResources().getStringArray(R.array.help_commandDescriptions);
    ...
    for (int i = 0; i < supportedCommands.length; ++i) {
      ...
      BrailleKeyBinding binding =
          BrailleKeyBindingUtils.getBrailleKeyBindingForCommand(command, sortedBindings);
      if (binding == null) {
        // No supported binding for this display. That's normal.
        continue;
      }
      addBindingForCommand(binding, descriptions[i], friendlyKeyNames, result);
    }

    KeyBindingsAdapter adapter =
        new KeyBindingsAdapter(
            KeyBindingsActivity.this, android.R.layout.simple_list_item_2, android.R.id.text1);
  ...
  private void addBindingForCommand(
    ...
    String keys =
        BrailleKeyBindingUtils.getFriendlyKeyNamesForCommand(binding, friendlyKeyNames, this);
