...
public class SearchTutorialActivity extends Activity
  ...
  public void onConnectionStateChanged(int state) {
    if (state == Display.STATE_CONNECTED) {
      ...
      TextView instructionsTextView = (TextView) findViewById(R.id.instructionsSpecific);
      instructionsTextView.setVisibility(View.INVISIBLE);
    }
  }

  /** Fills in the tutorial text view. Adds in key bindings as arguments if possible. */
  private void populateTutorialText() {
    ...
    BrailleKeyBinding nextItemBinding =
        BrailleKeyBindingUtils.getBrailleKeyBindingForCommand(
            BrailleInputEvent.CMD_NAV_ITEM_NEXT, sortedBindings);
    BrailleKeyBinding previousItemBinding =
        BrailleKeyBindingUtils.getBrailleKeyBindingForCommand(
            BrailleInputEvent.CMD_NAV_ITEM_PREVIOUS, sortedBindings);
    BrailleKeyBinding enterBinding =
        BrailleKeyBindingUtils.getBrailleKeyBindingForCommand(
            BrailleInputEvent.CMD_KEY_ENTER, sortedBindings);
    BrailleKeyBinding toggleSearchBinding =
        BrailleKeyBindingUtils.getBrailleKeyBindingForCommand(
            BrailleInputEvent.CMD_TOGGLE_INCREMENTAL_SEARCH, sortedBindings);

    if (nextItemBinding == null
        || previousItemBinding == null
        || enterBinding == null
        || toggleSearchBinding == null) {
    ...
    String enterName =
        BrailleKeyBindingUtils.getFriendlyKeyNamesForCommand(enterBinding, friendlyKeyNames, this);
  ...
  /**
   * Tries to start the activity. May do nothing if stored preferences say to not show the tutorial
   * again. Returns whether the tutorial was actually started.
   */
  ...
  /**
   * Closes the activity and saves a preference about whether or not to display the tutorial in the
   * future.
   */
  public void onClickCloseButton(View v) {
    final SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(this);
    CheckBox doNotShowCheckBox = (CheckBox) findViewById(R.id.doNotShowCheckBox);
    prefs.edit().putBoolean(PREF_DO_NOT_SHOW, doNotShowCheckBox.isChecked()).apply();
