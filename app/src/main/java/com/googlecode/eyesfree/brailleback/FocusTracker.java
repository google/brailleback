 ...
 *
 * <p>Currently, this class moves accessibility focus to the node that has input focus when the
 * screen is turned on or a display gets connected.
 */
public class FocusTracker extends BroadcastReceiver
  ...
  /**
   * Sets the accessibility focus to the node that currently has input focus, if accessibility focus
   * is not already set in the currently focused window.
   */
  private void setFocusFromInput() {
    ...
    try {
      ...
      if (inputFocused == null
          || !AccessibilityNodeInfoUtils.shouldFocusNode(mAccessibilityService, inputFocused)) {
        return;
      }
      inputFocused.performAction(AccessibilityNodeInfoCompat.ACTION_ACCESSIBILITY_FOCUS);
    } finally {
