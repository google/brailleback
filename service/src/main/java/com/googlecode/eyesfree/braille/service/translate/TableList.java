...
public class TableList {
  ...
  private static final Pattern LOCALE_SPLITTER = Pattern.compile("_", Pattern.LITERAL);
  ...
  private final Map<String, String> mTableFileNames = new HashMap<String, String>();
  ...
  private void parseTable(XmlResourceParser p) {
    ...
    if (locale == null) {
      throw new RuntimeException(p.getPositionDescription() + ": Locale must be specified");
    }
    int dots = p.getAttributeIntValue(null, ATTR_DOTS, -1);
    int grade = p.getAttributeIntValue(null, ATTR_GRADE, -1);
    if (dots < 0 && grade < 0) {
      throw new RuntimeException(
          p.getPositionDescription() + ": neither dots nor grade was specified");
    }
    if (grade >= 0 && dots < 0) {
      dots = 6;
    }
    switch (dots) {
      ...
      case 8:
        if (grade >= 0) {
          throw new RuntimeException(
              p.getPositionDescription() + ": grade must not be specified for 8 dot " + "braille");
        }
        break;
      default:
        throw new RuntimeException(p.getPositionDescription() + ": dots must be either 6 or 8");
    }
    String id = p.getAttributeValue(null, ATTR_ID);
    if (id == null) {
      throw new RuntimeException(p.getPositionDescription() + ": missing id attribute");
    }
    String variant = p.getAttributeValue(null, ATTR_VARIANT);
    String fileName = p.getAttributeValue(null, ATTR_FILE_NAME);
    if (fileName == null) {
      throw new RuntimeException(p.getPositionDescription() + ": missing fileName attribute");
    }
    ...
    if (DBG) {
      Log.v(
          LOG_TAG,
          String.format(
              "Table %s: locale=%s, dots=%d, " + "grade=%d, variant=%s,fileName=%s",
              id, locale.getDisplayName(), dots, grade, variant, fileName));
    }
  }
  ...
  private static Locale parseLocale(String value) {
    ...
    switch (pieces.length) {
      ...
      default:
        return new Locale(pieces[0], pieces[1], pieces[2]);
    }
  }
}
