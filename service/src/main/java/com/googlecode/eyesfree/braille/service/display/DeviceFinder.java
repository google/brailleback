...
public class DeviceFinder {
  ...
  private static final String LAST_CONNECTED_DEVICE_KEY = "lastBluetoothDevice";
  ...
  public static class DeviceInfo {
    ...
    /** Returns the service record uuid to use when connecting to this device. */
    public UUID getSdpUuid() {
      return mSdpUuid;
    }

    /**
     * Returns whether to connect securely (preferred) or not.
     *
  ...
  /** Returns a list of bonded and supported devices in the order they should be tried. */
      ...
      for (Pattern nameRegex : mNameRegexes) {
        if (nameRegex.matcher(name).lookingAt()) {
          return new DeviceInfo(
              bluetoothDevice, mDriverCode, SERIAL_BOARD_UUID, mConnectSecurely, mFriendlyKeyNames);
        }
      }
      return null;
    }
  ...
  private static class KeyNameMapBuilder {
    private final Map<String, Integer> mNameMap = new HashMap<String, Integer>();
