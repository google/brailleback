...
/**
 * An Andorid service that connects to braille displays and exposes a unified interface to other
 * Apps.
 *
 * <p>DisplayService contains ReadThread. ReadThread contains DriverThread and DeviceFinder.
...
public class DisplayService extends Service implements DriverThread.OnInputEventListener {
  ...
  /** Listener thread which reads from the braille device and forwards data to the driver. */
  ...
  /** Delay between the screen goes off and the display gets automatically disconnected. */
  ...
  /** Set to {@code true} if a connect request comes in while we are still disconnecting. */
  ...
  private class ServiceImpl extends IBrailleService.Stub {
    @Override
    public boolean registerCallback(final IBrailleServiceCallback callback) {
      if (callback == null) {
  ...
  /*package*/ void onDisplayConnected(final BrailleDisplayProperties properties) {
  ...
  /** Forwards input events from the driver thread to be broadcast from the main service thread. */
  @Override
  ...
  /**
   * Disconnects the service from the currently connected braille device, sends notification to the
   * clients about the state change.
   */
  ...
  /**
   * Tries to connect to the braille device, on success sends notification to registered clients.
   */
  private void connectBraille() {
    ...
    if (mReadThread != null) {
      ...
      mReadThread = new ReadThread(this, mTablesDir, mPendingBluetoothAddress);
      mPendingBluetoothAddress = null;
      mReadThread.start();
    }
  }

  /** If {@code intent} indicates that a new device has been paired, try to connect to it. */
  private void connectFromBroadcastIntent(Intent intent) {
    ...
    BluetoothDevice bthDev = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
    if (bthDev == null) {
      return;
    }
    ...
    try {
      switch (mConnectionState) {
        ...
        case STATE_DISCONNECTED:
          ...
          break;
        default:
          Log.e(LOG_TAG, "Unknown connection state: " + mConnectionState);
          break;
      }
  ...
  /**
   * Returns {@code true} if there are any registered clients that haven't died or unregistered
   * themselves.
   */
  ...
  private void ensureDataFiles() {
    ...
    // service starts.
    ZipResourceExtractor extractor =
        new ZipResourceExtractor(this, R.raw.keytables, mTablesDir) {
          @Override
  ...
  private class MainHandler extends Handler {
    ...
    public void setConnectionProgress(String description) {
      obtainMessage(MSG_SET_CONNECTION_PROGRESS, description).sendToTarget();
    }
    ...
    public void handleMessage(Message msg) {
      switch (msg.what) {
        ...
        case MSG_ON_DISPLAY_CONNECTED:
          handleOnDisplayConnected((BrailleDisplayProperties) msg.obj);
          break;
    ...
    private void handleRegisterCallback(IBrailleServiceCallback callback) {
      ...
      if (mDataFileState == FILES_NOT_EXTRACTED
          || (mConnectionState == STATE_DISCONNECTED && mReadThread != null)) {
    ...
    private void handleOnDisplayConnected(BrailleDisplayProperties properties) {
    ...
    private void handleSetConnectionProgress(String description) {
      if ((description == null && mConnectionProgress == null)
          || (description != null && description.equals(mConnectionProgress))) {
        return;
      }
