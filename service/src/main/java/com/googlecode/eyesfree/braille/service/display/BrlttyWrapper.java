 ...
 *
 * <p>This class prvoides a low-level interface to the functionality of brltty that is used to
 * control braille displays.
 *
 * <p>Since brltty is single-threaded and relies on some global state, there can only be one
 * instance of this class active at any given time. In addition, after construction, all method
 * calls must be made from one thread (which may be different from the thread used to construct the
 * object). The one exception to this rules is {@link #addBytesFromDevice()}, which can be called
 * from any thread.
 */
public class BrlttyWrapper {
  ...
  /**
   * Constructs a {@link BrlttyWrapper}. {@code driverThread} is used to send data bytes to the
   * device.
   */
  public BrlttyWrapper(
  ...
  /** Initializes the driver, making sure there is a device connected. */
  public boolean start() {
    return startNative(mDeviceInfo.getDriverCode(), deviceBrlttyAddress());
  }

  /** Uninitializes the driver and cleans up any resources allocated by this object. */
  public void stop() {
    stopNative();
  }
  ...
  /**
   * Sets the content of the braille display to {@code pattern}, where each byte represents one
   * braille cell. {@code pattern} should be as large as the total number of cells on the display.
   */
  ...
  /**
   * Polls the driver for a single key command, returning -1 if no command is available.
   *
   * <p>This is typically used when more input data from the device has been detected. This call
   * does not block indefinitely, but it can still block for small intervals while waiting for more
  ...
  /**
   * Adds more data to be consumed by the driver. This method can be called at any time after an
   * object of this class has been constructed until {@link #stop()} is called. It can be called
   * from any thread. In particular, it is important to keep calling this method during other calls
   * that might consume input so that they can proceed and consume that input.
   */
  public void addBytesFromDevice(byte[] bytes, int size) throws IOException {
    addBytesFromDeviceNative(bytes, size);
  }

  /** Returns the address of the connected device in the form expected by the brltty drivers. */
  private String deviceBrlttyAddress() {
    return String.format("bluetooth:%s", mDeviceInfo.getBluetoothDevice().getAddress());
  }

  private BrailleKeyBinding[] getKeyMap() {
    BrailleKeyBinding[] fullKeyMap = getKeyMapNative();
    ArrayList<BrailleKeyBinding> arrayList = new ArrayList<BrailleKeyBinding>(fullKeyMap.length);
    ...
    BrailleKeyBinding[] relevantBindings = arrayList.toArray(fullKeyMap);
  ...
  private native boolean startNative(String driverCode, String brailleDevice);
