...
/**
 * Mapping between braille dot and textual representation as used in dots
 * operands
 */
...
typedef struct {
  ...
  TranslationTableCharacterAttributes
      before;                      /** character types which must precede */
...
typedef struct { /* translation table */
  ...
  TranslationTableOffset
      ruleArea[1]; /** Space for storing all rules and values */
 ...
 */
char **EXPORT_CALL _lou_defaultTableResolver(const char *tableList,
                                             const char *base);
...
int EXPORT_CALL _lou_translateWithTracing(
    const char *tableList, const widechar *inbuf, int *inlen, widechar *outbuf,
    int *outlen, formtype *typeform, char *spacing, int *outputPos,
    int *inputPos, int *cursorPos, int mode, const TranslationTableRule **rules,
    int *rulesLen);

int EXPORT_CALL _lou_backTranslateWithTracing(
    const char *tableList, const widechar *inbuf, int *inlen, widechar *outbuf,
    int *outlen, formtype *typeform, char *spacing, int *outputPos,
    int *inputPos, int *cursorPos, int mode, const TranslationTableRule **rules,
    int *rulesLen);
...
int EXPORT_CALL _lou_pattern_compile(const widechar *input, const int input_max,
                                     widechar *expr_data, const int expr_max,
                                     const TranslationTableHeader *t);

void EXPORT_CALL _lou_pattern_reverse(widechar *expr_data);

int EXPORT_CALL _lou_pattern_check(const widechar *input, const int input_start,
                                   const int input_minmax, const int input_dir,
 ...
 */
void EXPORT_CALL _lou_logWidecharBuf(logLevels level, const char *msg,
                                     const widechar *wbuf, int wlen);
