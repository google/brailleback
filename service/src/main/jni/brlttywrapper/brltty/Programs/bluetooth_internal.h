...
extern ssize_t bthGetData(BluetoothConnectionExtension *bcx, void *buffer,
                          size_t size, int initialTimeout,
                          int subsequentTimeout);

extern ssize_t bthPutData(BluetoothConnectionExtension *bcx, const void *buffer,
                          size_t size);

extern int bthDiscoverChannel(uint8_t *channel,
                              BluetoothConnectionExtension *bcx,
                              const void *uuidBytes, size_t uuidLength,
                              int timeout);

extern int bthOpenChannel(BluetoothConnectionExtension *bcx, uint8_t channel,
                          int timeout);
