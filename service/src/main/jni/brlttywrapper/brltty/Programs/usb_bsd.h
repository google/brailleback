...
static int usbSetTimeout(int file, int new, int *old) {
  if (!old || (new != *old)) {
    int arg = new;
...
static int usbSetShortTransfers(int file, int arg) {
  if (ioctl(file, USB_SET_SHORT_XFER, &arg) != -1) return 1;
  logSystemError("USB set short transfers");
  return 0;
}

int usbDisableAutosuspend(UsbDevice *device) {
  logUnsupportedFunction();
  return 0;
}

int usbSetConfiguration(UsbDevice *device, unsigned char configuration) {
...
int usbClaimInterface(UsbDevice *device, unsigned char interface) {
  return 1;
  /*
    logUnsupportedFunction();
    return 0;
...
int usbReleaseInterface(UsbDevice *device, unsigned char interface) {
  return 1;
  /*
    logUnsupportedFunction();
    return 0;
...
int usbSetAlternative(UsbDevice *device, unsigned char interface,
...
int usbClearHalt(UsbDevice *device, unsigned char endpointAddress) {
  logUnsupportedFunction();
  return 0;
}

ssize_t usbControlTransfer(UsbDevice *device, uint8_t direction,
                           uint8_t recipient, uint8_t type, uint8_t request,
...
void *usbSubmitRequest(UsbDevice *device, unsigned char endpointAddress,
                       void *buffer, size_t length, void *context) {
  logUnsupportedFunction();
  return NULL;
}

int usbCancelRequest(UsbDevice *device, void *request) {
  logUnsupportedFunction();
  return 0;
}

void *usbReapResponse(UsbDevice *device, unsigned char endpointAddress,
                      UsbResponse *response, int wait) {
  logUnsupportedFunction();
  return NULL;
}

int usbMonitorInputEndpoint(UsbDevice *device, unsigned char endpointNumber,
                            AsyncMonitorCallback *callback, void *data) {
  return 0;
}

ssize_t usbReadEndpoint(UsbDevice *device, unsigned char endpointNumber,
...
ssize_t usbWriteEndpoint(UsbDevice *device, unsigned char endpointNumber,
...
int usbReadDeviceDescriptor(UsbDevice *device) {
  UsbDeviceExtension *devx = device->extension;
...
int usbAllocateEndpointExtension(UsbEndpoint *endpoint) {
  ...
  if ((eptx = malloc(sizeof(*eptx)))) {
    ...
    snprintf(path, sizeof(path), USB_ENDPOINT_PATH_FORMAT, length, prefix,
             USB_ENDPOINT_NUMBER(endpoint->descriptor));
...
void usbDeallocateEndpointExtension(UsbEndpointExtension *eptx) {
  if (eptx->file != -1) {
    close(eptx->file);
    eptx->file = -1;
  }
...
void usbDeallocateDeviceExtension(UsbDeviceExtension *devx) {
  if (devx->file != -1) {
    close(devx->file);
    devx->file = -1;
  }
...
UsbDevice *usbFindDevice(UsbDeviceChooser *chooser,
      ...
      for (deviceNumber = 1; deviceNumber < USB_MAX_DEVICES; deviceNumber++) {
        ...
        if (ioctl(bus, USB_DEVICEINFO, &info) != -1) {
          ...
          logMessage(LOG_CATEGORY(USB_IO),
                     "device [%d,%d]: vendor=%s product=%s", busNumber,
                     deviceNumber, info.udi_vendor, info.udi_product);
          {
            int nameNumber;
            for (nameNumber = 0; nameNumber < USB_MAX_DEVNAMES; nameNumber++) {
              const char *name = info.udi_devnames[nameNumber];
              if (*name)
                logMessage(LOG_CATEGORY(USB_IO), "name %d: %s", nameNumber,
                           name);
            }
          }
          ...
          if (strncmp(deviceName, driver, strlen(driver)) == 0) {
            ...
            snprintf(devicePath, sizeof(devicePath), USB_CONTROL_PATH_FORMAT,
                     deviceName);
