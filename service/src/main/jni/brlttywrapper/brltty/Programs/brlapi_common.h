...
static ssize_t brlapi_writeFile(brlapi_fileDescriptor fd, const void *buffer,
                                size_t size) {
...
static ssize_t brlapi_readFile(brlapi_fileDescriptor fd, void *buffer,
...
#endif            /* __MINGW32__ */
    if (res == 0) /* Unexpected end of file ! */
      break;
  }
  return n;
}
...
ssize_t BRLAPI(writePacket)(brlapi_fileDescriptor fd, brlapi_packetType_t type,
...
ssize_t BRLAPI(readPacketHeader)(brlapi_fileDescriptor fd,
                                 brlapi_packetType_t *packetType) {
  uint32_t header[2];
  ssize_t res;
  if ((res = brlapi_readFile(fd, header, sizeof(header), 0)) !=
      sizeof(header)) {
    if (res < 0) {
      /* reports EINTR too */
...
ssize_t BRLAPI(readPacketContent)(brlapi_fileDescriptor fd, size_t packetSize,
  ...
  if (res < MIN(bufSize, packetSize))
    return -2; /* pkt smaller than announced => EOF */
...
ssize_t BRLAPI(readPacket)(brlapi_fileDescriptor fd,
                           brlapi_packetType_t *packetType, void *buf,
                           size_t size) {
...
static int BRLAPI(loadAuthKey)(const char *filename, size_t *authlength,
                               void *auth) {
  int fd;
  off_t stsize;
  struct stat statbuf;
...
static int BRLAPI(expandHost)(const char *hostAndPort, char **host,
                              char **port) {
  const char *c;
...
static int BRLAPI(getArgumentWidth)(brlapi_keyCode_t keyCode) {
...
static int BRLAPI(getKeyrangeMask)(brlapi_rangeType_t r, brlapi_keyCode_t code,
...
static char *BRLAPI(getKeyFile)(const char *auth) {
  const char *path;
  char *ret, *delim;
