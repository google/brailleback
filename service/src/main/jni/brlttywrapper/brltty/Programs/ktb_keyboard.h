...
#define KBD_KEY_VALUE(grp, num) \
  { .group = KBD_GROUP(grp), .number = num }
#define KBD_KEY_ENTRY(grp, key) KBD_KEY_VALUE(grp, KBD_KEY(grp, key))

#define KBD_GROUP_NAME(grp, nam) \
  { .value = KBD_KEY_VALUE(grp, KTB_KEY_ANY), .name = nam }
#define KBD_KEY_NAME(grp, key, nam) \
  { .value = KBD_KEY_ENTRY(grp, key), .name = nam }
