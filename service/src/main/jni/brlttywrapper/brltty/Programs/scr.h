...
extern int readScreen(short left, short top, short width, short height,
                      ScreenCharacter *buffer);
extern int readScreenText(short left, short top, short width, short height,
                          wchar_t *buffer);
...
static inline int readScreenRows(int row, int width, int height,
                                 ScreenCharacter *buffer) {
  return readScreen(0, row, width, height, buffer);
}

static inline int readScreenRow(int row, int width, ScreenCharacter *buffer) {
...
extern const DriverDefinition *getScreenDriverDefinition(
    const ScreenDriver *driver);

extern int haveScreenDriver(const char *code);
extern const char *getDefaultScreenDriver(void);

extern const ScreenDriver *loadScreenDriver(const char *code,
                                            void **driverObject,
