...
typedef struct {
  ...
  ** It is a number between 0 and 3. 0 = 'K' 1 = 'L' etc... */
  ...
  /* cette variable dit si on est en train de changer le type de transfert
     (br->pc ou pc->br, si on change l'extension ou si on change le nom du
     fichier. */
  unsigned char status;
  /* Cette variable dit si on est en train de transférer du terminal vers le pc
   * ou pas */
  unsigned char brpc;
  ...
  struct dirent **list;
  /* Pour le transfert pc->br, un descripteur de fichier pointant sur le fichier
   * à transférer */
  int fd;
