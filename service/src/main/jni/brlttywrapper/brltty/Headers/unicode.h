...
#define UNICODE_CHARACTER_MASK                                 \
  (UNICODE_CELL_MASK | UNICODE_ROW_MASK | UNICODE_PLANE_MASK | \
   UNICODE_GROUP_MASK)
...
#define UNICODE_PLANE_NUMBER(c) \
  (((c)&UNICODE_PLANE_MASK) >> UNICODE_PLANE_SHIFT)
#define UNICODE_GROUP_NUMBER(c) \
  (((c)&UNICODE_GROUP_MASK) >> UNICODE_GROUP_SHIFT)
#define UNICODE_CHARACTER(group, plane, row, cell)                       \
  (((group) << UNICODE_GROUP_SHIFT) | ((plane) << UNICODE_PLANE_SHIFT) | \
   ((row) << UNICODE_ROW_SHIFT) | ((cell) << UNICODE_CELL_SHIFT))
...
extern int handleBestCharacter(wchar_t character,
                               CharacterHandler handleCharacter, void *data);
