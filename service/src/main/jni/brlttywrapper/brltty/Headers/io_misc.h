...
extern ssize_t readFile(FileDescriptor fileDescriptor, void *buffer,
                        size_t size, int initialTimeout, int subsequentTimeout);

extern ssize_t writeFile(FileDescriptor fileDescriptor, const void *buffer,
                         size_t size);
...
extern ssize_t readSocket(SocketDescriptor socketDescriptor, void *buffer,
                          size_t size, int initialTimeout,
                          int subsequentTimeout);

extern ssize_t writeSocket(SocketDescriptor socketDescriptor,
                           const void *buffer, size_t size);

extern int connectSocket(SocketDescriptor socketDescriptor,
                         const struct sockaddr *address, size_t addressLength,
                         int timeout);

static inline int getSocketOption(SocketDescriptor socketDescriptor, int level,
                                  int option, void *value, socklen_t *length) {
  return getsockopt(socketDescriptor, level, option, value, length);
}
#endif /* GOT_SOCKETS */

extern int changeOpenFlags(FileDescriptor fileDescriptor, int flagsToClear,
                           int flagsToSet);
