...
typedef enum { UsbEndpointNumber_Mask = 0X0F } UsbEndpointNumber;
#define USB_ENDPOINT_NUMBER(descriptor) \
  ((descriptor)->bEndpointAddress & UsbEndpointNumber_Mask)
...
#define USB_ENDPOINT_DIRECTION(descriptor) \
  ((descriptor)->bEndpointAddress & UsbEndpointDirection_Mask)
...
#define USB_ENDPOINT_TRANSFER(descriptor) \
  ((descriptor)->bmAttributes & UsbEndpointTransfer_Mask)
...
#define USB_ENDPOINT_ISOCHRONOUS(descriptor) \
  ((descriptor)->bmAttributes & UsbEndpointIsochronous_Mask)
...
typedef struct {
  ...
  UsbClassDescriptor
      descriptors[(0XFF - UsbDescriptorSize_HID) / UsbDescriptorSize_Class];
...
#define BEGIN_USB_CHANNEL_DEFINITIONS \
  static const UsbChannelDefinition usbChannelDefinitions[] = {
#define END_USB_CHANNEL_DEFINITIONS \
  { .vendor = 0 }                   \
  }                                 \
  ;
...
typedef struct {
  ...
  int (*setLineConfiguration)(UsbDevice *device, unsigned int baud,
                              unsigned int dataBits, SerialStopBits stopBits,
                              SerialParity parity,
                              SerialFlowControl flowControl);
  int (*setLineProperties)(UsbDevice *device, unsigned int baud,
                           unsigned int dataBits, SerialStopBits stopBits,
                           SerialParity parity);
  int (*setBaud)(UsbDevice *device, unsigned int baud);
  int (*setDataFormat)(UsbDevice *device, unsigned int dataBits,
                       SerialStopBits stopBits, SerialParity parity);
