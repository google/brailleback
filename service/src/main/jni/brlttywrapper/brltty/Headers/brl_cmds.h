...
#define BRL_CODE_GET(name, code) \
  (((code) >> BRL_SHIFT_##name) & BRL_CODE_MASK(name))
#define BRL_CODE_PUT(name, value) ((value) << BRL_SHIFT_##name)
#define BRL_CODE_SET(name, value) \
  BRL_CODE_PUT(name, ((value)&BRL_CODE_MASK(name)))
...
#define BRL_ARG_GET(code) \
  (BRL_CODE_GET(ARG, (code)) | (BRL_CODE_GET(EXT, (code)) << BRL_BITS_ARG))
#define BRL_ARG_SET(value) \
  (BRL_CODE_SET(ARG, (value)) | BRL_CODE_SET(EXT, ((value) >> BRL_BITS_ARG)))
...
#define BRL_FLG_TOGGLE_MASK \
  (BRL_FLG_TOGGLE_ON | BRL_FLG_TOGGLE_OFF) /* mask for all toggle flags */

#define BRL_FLG_MOTION_ROUTE \
  0X040000 /* bring screen cursor into braille window after function */
#define BRL_FLG_MOTION_SCALED \
  0X080000 /* scale arg=0X00-0XFF to screen height */
