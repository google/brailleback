...
typedef enum { sbwAll, sbwEndOfLine, sbwRestOfLine } SkipBlankWindowsMode;

typedef enum { sayImmediate, sayEnqueue } SayMode;

typedef enum { sucNone, sucSayCap, sucRaisePitch } SpeechUppercaseIndicator;

typedef enum { swsNone, swsSaySpace } SpeechWhitespaceIndicator;

typedef enum { tf24Hour, tf12Hour } TimeFormat;

typedef enum { tsColon, tsDot } TimeSeparator;

typedef enum { dpNone, dpBeforeTime, dpAfterTime } DatePosition;

typedef enum { dfYearMonthDay, dfMonthDayYear, dfDayMonthYear } DateFormat;

typedef enum { dsDash, dsSlash, dsDot } DateSeparator;

typedef enum { spNone, spLeft, spRight } StatusPosition;
...
typedef enum { atOff, at5s, at10s, at20s, at40s } AutoreleaseTime;

typedef enum { ctdNone, ctd250ms, ctd500ms, ctd1s, ctd2s } CursorTrackingDelay;
