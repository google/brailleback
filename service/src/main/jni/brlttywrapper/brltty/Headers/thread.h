...
extern int createThread(const char *name, pthread_t *thread,
                        const pthread_attr_t *attributes,
                        ThreadFunction *function, void *argument);

extern int callThreadFunction(const char *name, ThreadFunction *function,
...
static inline void enterCriticalSection(CriticalSectionLock *lock) {
  pthread_mutex_lock(lock);
}

static inline void leaveCriticalSection(CriticalSectionLock *lock) {
...
#define THREAD_SPECIFIC_DATA_INITIALIZER() .data = NULL
#endif /* thread specific data */
} ThreadSpecificDataControl;

#define THREAD_SPECIFIC_DATA_CONTROL(name)                            \
  static ThreadSpecificDataControl name = {.new = name##_new,         \
