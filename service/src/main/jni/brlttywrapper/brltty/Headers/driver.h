...
#define DRIVER_DEFINITION_INITIALIZER             \
  .definition = {.name = STRINGIFY(DRIVER_NAME),  \
...
#define DRIVER_VERSION_DECLARATION(type) \
  const char CONCATENATE(type##_version_, DRIVER_CODE)[] = DRIVER_VERSION_STRING
...
extern void logInputProblem(const char *problem, const unsigned char *bytes,
                            size_t count);
