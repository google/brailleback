...
extern int setTableDataVariables(const char *tableExtension,
                                 const char *subtableExtension);
...
typedef enum { DFO_NO_COMMENTS = 0X01 } DataFileOptions;
...
extern int processDataFile(const char *name,
                           const DataFileParameters *parameters);
extern void reportDataError(DataFile *file, char *format, ...) PRINTF(2, 3);

extern int processDataStream(DataFile *includer, FILE *stream, const char *name,
                             const DataFileParameters *parameters);

extern int compareKeyword(const wchar_t *keyword, const wchar_t *characters,
                          size_t count);
extern int compareKeywords(const wchar_t *keyword1, const wchar_t *keyword2);

extern int isKeyword(const wchar_t *keyword, const wchar_t *characters,
                     size_t count);
...
extern int getDataOperand(DataFile *file, DataOperand *operand,
                          const char *description);
extern int getTextOperand(DataFile *file, DataOperand *text,
                          const char *description);
...
extern int parseDataString(DataFile *file, DataString *string,
                           const wchar_t *characters, int length,
                           int noUnicode);
extern int getDataString(DataFile *file, DataString *string, int noUnicode,
                         const char *description);
...
extern int writeEscapedCharacters(FILE *stream, const wchar_t *characters,
                                  size_t count);
...
extern int parseCellsOperand(DataFile *file, ByteOperand *cells,
                             const wchar_t *characters, int length);
extern int getCellsOperand(DataFile *file, ByteOperand *cells,
                           const char *description);

extern int writeDots(FILE *stream, unsigned char cell);
extern int writeDotsCell(FILE *stream, unsigned char cell);
extern int writeDotsCells(FILE *stream, const unsigned char *cells,
                          size_t count);

extern int writeUtf8Cell(FILE *stream, unsigned char cell);
extern int writeUtf8Cells(FILE *stream, const unsigned char *cells,
                          size_t count);
...
#define END_DATA_DIRECTIVE_TABLE                              \
  ...
  static DataDirectives directives = {                        \
      .unsorted = {.table = unsortedDirectives,               \
                   .count = ARRAY_COUNT(unsortedDirectives)}, \
                                                              \
      .sorted = {.table = NULL, .count = 0},                  \
                                                              \
      .unnamed = NULL};

extern int processDirectiveOperand(DataFile *file, DataDirectives *directives,
                                   const char *description, void *data);

#define DATA_CONDITION_TESTER(name) \
  int name(DataFile *file, const DataOperand *identifier, void *data)
typedef DATA_CONDITION_TESTER(DataConditionTester);

extern int processConditionOperands(DataFile *file,
                                    DataConditionTester *testCondition,
                                    int negateCondition,
                                    const char *description, void *data);
...
#define DATA_CONDITION_DIRECTIVES                             \
  {.name = WS_C("else"),                                      \
   .processor = processElseOperands,                          \
   .unconditional = 1},                                       \
  {                                                           \
    .name = WS_C("endif"), .processor = processEndIfOperands, \
    .unconditional = 1                                        \
  }
...
#define DATA_VARIABLE_DIRECTIVES                               \
  {.name = WS_C("ifvar"),                                      \
   .processor = processIfVarOperands,                          \
   .unconditional = 1},                                        \
      {.name = WS_C("ifnotvar"),                               \
       .processor = processIfNotVarOperands,                   \
       .unconditional = 1},                                    \
      {.name = WS_C("beginvariables"),                         \
       .processor = processBeginVariablesOperands},            \
      {.name = WS_C("endvariables"),                           \
       .processor = processEndVariablesOperands},              \
      {.name = WS_C("listvariables"),                          \
       .processor = processListVariablesOperands},             \
      {.name = WS_C("assigndefault"),                          \
       .processor = processAssignDefaultOperands},             \
  {                                                            \
