...
static inline void swapBytes(unsigned char *byte1, unsigned char *byte2) {
...
#define PUT_ENDIAN_FUNCTION(which, bits)                          \
  static inline void put##which##Endian##bits(uint##bits##_t *to, \
                                              uint##bits##_t from)

#define DEFINE_PHYSICAL_ENDIAN_FUNCTIONS(bits)                             \
  GET_ENDIAN_FUNCTION(Native, bits) { return from; }                       \
                                                                           \
  GET_ENDIAN_FUNCTION(Other, bits) {                                       \
    ...
    return overlay.u##bits;                                                \
                                                                           ...
                                                                           \
  PUT_ENDIAN_FUNCTION(Native, bits) { *to = getNativeEndian##bits(from); } \
                                                                           \
  PUT_ENDIAN_FUNCTION(Other, bits) { *to = getOtherEndian##bits(from); }
...
#define DEFINE_ENDIAN_FUNCTIONS_FOR_BITS(bits, logical, physical) \
  GET_ENDIAN_FUNCTION(logical, bits) {                            \
    return get##physical##Endian##bits(from);                     \
                                                                  ...
                                                                  \
  PUT_ENDIAN_FUNCTION(logical, bits) { put##physical##Endian##bits(to, from); }
