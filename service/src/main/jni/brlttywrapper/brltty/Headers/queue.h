...
typedef int ItemComparator(const void *newItem, const void *existingItem,
                           void *queueData);

extern Queue *newQueue(ItemDeallocator *deallocateItem,
                       ItemComparator *compareItems);
extern void deallocateQueue(Queue *queue);

typedef Queue *QueueCreator(void *data);
extern Queue *getProgramQueue(Queue **queue, const char *name, int create,
...
extern Element *findElement(const Queue *queue, ItemTester *testItem,
                            void *data);
...
extern Element *processQueue(Queue *queue, ItemProcessor *processItem,
                             void *data);
