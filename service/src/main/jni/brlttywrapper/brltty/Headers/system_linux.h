...
extern int openCharacterDevice(const char *name, int flags, int major,
                               int minor);
...
extern int writeInputEvent(UinputObject *uinput, uint16_t type, uint16_t code,
                           int32_t value);
...
extern InputEventMonitor *newInputEventMonitor(
    const char *name, UinputObjectPreparer *prepareUinputObject,
    InputEventHandler *handleInputEvent);

extern void destroyInputEventMonitor(InputEventMonitor *monitor);
