...
#define IS_WITHIN_RANGE(index, start, end) \
...
#define FUNCTION_TYPEDEF(functionName, returnType, argumentList)      \
  typedef FUNCTION_DECLARATION(SYMBOL_TYPE(functionName), returnType, \
                               argumentList)
...
#include <unistd.h>
    ...
    ((__MINGW32_MAJOR_VERSION == 3) && (__MINGW32_MINOR_VERSION < 10))
...
#define setErrno(error) \
  errno = cygwin_internal(CW_GET_ERRNO_FROM_WINERROR, (error))
#else /* __CYGWIN__ */
...
#define wmemchr(source, character, count) \
  memchr((const char *)(source), (char)(character), (count))
#define wmemcmp(source1, source2, count) \
  memcmp((const char *)(source1), (const char *)(source2), (count))
#define wmemcpy(target, source, count) \
  memcpy((char *)(target), (const char *)(source), (count))
#define wmemmove(target, source, count) \
  memmove((char *)(target), (const char *)(source), (count))
#define wmemset(target, character, count) \
  memset((char *)(target), (char)(character), (count))

#define wcscasecmp(source1, source2) \
  strcasecmp((const char *)(source1), (const char *)(source2))
#define wcsncasecmp(source1, source2, count) \
  strncasecmp((const char *)(source1), (const char *)(source2), (count))
#define wcscat(target, source) strcat((char *)(target), (const char *)(source))
#define wcsncat(target, source, count) \
  strncat((char *)(target), (const char *)(source), (count))
#define wcscmp(source1, source2) \
  strcmp((const char *)(source1), (const char *)(source2))
#define wcsncmp(source1, source2, count) \
  strncmp((const char *)(source1), (const char *)(source2), (count))
#define wcscpy(target, source) strcpy((char *)(target), (const char *)(source))
#define wcsncpy(target, source, count) \
  strncpy((char *)(target), (const char *)(source), (count))
#define wcslen(source) strlen((const char *)(source))
#define wcsnlen(source, count) strnlen((const char *)(source), (count))

#define wcschr(source, character) \
  ((wchar_t *)strchr((const char *)(source), (char)(character)))
#define wcscoll(source1, source2) \
  strcoll((const char *)(source1), (const char *)(source2))
#define wcscspn(source, reject) \
  strcspn((const char *)(source), (const char *)(reject))
#define wcsdup(source) strdup((const char *)(source))
#define wcspbrk(source, accept) \
  strpbrk((const char *)(source), (const char *)(accept))
#define wcsrchr(source, character) \
  strrchr((const char *)(source), (char)(character))
#define wcsspn(source, accept) \
  strspn((const char *)(source), (const char *)(accept))
#define wcsstr(source, substring) \
  strstr((const char *)(source), (const char *)(substring))
#define wcstok(target, delimiters, end) \
  ((wchar_t *)strtok(((char *)(target)), ((const char *)(delimiters))))
#define wcswcs(source, substring) \
  strstr((const char *)(source), (const char *)(substring))
#define wcsxfrm(target, source, count) \
  strxfrm((char *)(target), (const char *)(source), (count))
#define wcstoul(nptr, endptr, base) \
  strtoul(((const char *)(nptr)), ((char **)(endptr)), (base))

#define wcstol(source, end, base) \
  strtol((const char *)(source), (char **)(end), (base))
#define wcstoll(source, end, base) \
  strtoll((const char *)(source), (char **)(end), (base))
...
#define swprintf(target, count, source, ...) \
  snprintf((char *)(target), (count), (const char *)(source), ##__VA_ARGS__)
#define vswprintf(target, count, source, args) \
  vsnprintf((char *)(target), (count), (const char *)(source), (args))

typedef unsigned char mbstate_t;

static inline size_t mbrtowc(wchar_t *pwc, const char *s, size_t n,
                             mbstate_t *ps) {
  if (!s) return 0;
  if (!n) return 0;
  if (pwc) *pwc = *s & 0XFF;
...
static inline int mbsinit(const mbstate_t *ps) { return 1; }
...
/* AC_CHECK_FUNC() is checking local libraries - these are the errors that
 * matter */
#undef HAVE_FCHDIR
#undef HAVE_SELECT
#endif /* GRUB_RUNTIME */
