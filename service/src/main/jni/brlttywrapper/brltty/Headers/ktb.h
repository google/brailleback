...
extern KeyTable *compileKeyTable(const char *name,
                                 KEY_NAME_TABLES_REFERENCE keys);
extern void destroyKeyTable(KeyTable *table);

typedef int KeyNameEntryHandler(const KeyNameEntry *kne, void *data);
extern int forEachKeyName(KEY_NAME_TABLES_REFERENCE keys,
                          KeyNameEntryHandler *handleKeyNameEntry, void *data);

typedef int KeyTableWriteLineMethod(const wchar_t *line, void *data);
typedef int KeyTableWriteHeaderMethod(const wchar_t *text, unsigned int level,
                                      void *data);
...
extern int listKeyTable(KeyTable *table, const KeyTableListMethods *methods,
                        KeyTableWriteLineMethod *writeLine, void *data);
extern int listKeyNames(KEY_NAME_TABLES_REFERENCE keys,
                        KeyTableWriteLineMethod *writeLine, void *data);
...
extern char *makeInputTablePath(const char *directory, const char *driver,
                                const char *name);

extern void resetKeyTable(KeyTable *table);
extern void releaseAllKeys(KeyTable *table);

extern KeyTableState processKeyEvent(KeyTable *table, unsigned char context,
                                     KeyGroup keyGroup, KeyNumber keyNumber,
                                     int press);
...
extern void getKeyGroupCommands(KeyTable *table, KeyGroup group, int *commands,
                                unsigned int size);
