...
again:
  ...
  if (res) {
    DWORD _res;
    if (GetExitCodeThread(thread, &_res)) *res = (void *)(DWORD_PTR)_res;
  }
  return 0;
}
...
static inline int pthread_mutexattr_settype(pthread_mutexattr_t *attr,
                                            int type) {
  if (type != PTHREAD_MUTEX_RECURSIVE) return EINVAL;
  *attr = type;
  return 0;
}
...
static inline int pthread_mutex_init(pthread_mutex_t *mutex,
  ...
  if (attr && *attr != PTHREAD_MUTEX_RECURSIVE) return EINVAL;
...
static inline int __pthread_mutex_alloc_concurrently(pthread_mutex_t *mutex) {
  ...
  if (!*mutex) winPthreadAssertPthread(pthread_mutex_init(mutex, NULL));
...
static inline int pthread_mutex_lock(pthread_mutex_t *mutex) {
  if (!*mutex) __pthread_mutex_alloc_concurrently(mutex);
again:
...
static inline int pthread_cond_timedwait(pthread_cond_t *cond,
  ...
  if (!cond->sem) winPthreadAssertPthread(pthread_cond_init(cond, NULL));
  cond->nbwait++;
...
static inline int pthread_cond_signal(pthread_cond_t *cond) {
  if (!cond->sem) winPthreadAssertPthread(pthread_cond_init(cond, NULL));
  if (cond->nbwait) ReleaseSemaphore(cond->sem, 1, NULL);
  return 0;
}
