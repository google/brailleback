...
#define STR_ADJUST(length)                                \
  do {                                                    \
    if ((strNext += (length)) > strEnd) strNext = strEnd; \
  } while (0)
...
#define STR_PRINTF(...) STR_FORMAT(snprintf, __VA_ARGS__)

#define STR_VPRINTF(format, arguments) STR_FORMAT(vsnprintf, format, arguments)
