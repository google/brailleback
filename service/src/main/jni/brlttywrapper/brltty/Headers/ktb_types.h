...
#define KEY_NAME_TABLE_DECLARATION(name) \
  const KeyNameEntry KEY_NAME_TABLE(name)[]
#define KEY_NAME_ENTRY(keyNumber, keyName) \
  { .value.number = keyNumber, .name = keyName }
#define KEY_GROUP_ENTRY(keyGroup, keyName) \
  { .value = {.group = keyGroup, .number = KTB_KEY_ANY}, .name = keyName }
#define LAST_KEY_NAME_ENTRY \
  { .name = NULL }
...
#define END_KEY_NAME_TABLE \
  ...
  ;
#define KEY_NAME_SUBTABLE(name, count) \
  (KEY_NAME_TABLE(name) + (ARRAY_COUNT(KEY_NAME_TABLE(name)) - 1 - (count)))

#define KEY_NAME_TABLES(name) keyNameTables_##name
#define KEY_NAME_TABLES_REFERENCE const KeyNameEntry *const *
#define KEY_NAME_TABLES_DECLARATION(name) \
  const KeyNameEntry *const KEY_NAME_TABLES(name)[]
...
#define KEY_TABLE_DECLARATION(name) \
  const KeyTableDefinition KEY_TABLE_DEFINITION(name)
#define KEY_TABLE_INITIALIZER(name) \
