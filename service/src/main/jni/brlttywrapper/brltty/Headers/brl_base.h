...
#define KEY_TABLE_LIST_DECLARATION \
  const KeyTableDefinition *const KEY_TABLE_LIST_SYMBOL[]
...
#define END_KEY_TABLE_LIST  \
  LAST_KEY_TABLE_DEFINITION \
  }                         \
  ;
...
#define BRL_KEY_GROUP_ENTRY(drv, grp, nam) \
  KEY_GROUP_ENTRY(BRL_KEY_GROUP(drv, grp), nam)
#define BRL_KEY_NUMBER_ENTRY(drv, grp, num, nam) \
  { .value = {.group = BRL_KEY_GROUP(drv, grp), .number = (num)}, .name = nam }
#define BRL_KEY_NAME_ENTRY(drv, grp, key, nam) \
  BRL_KEY_NUMBER_ENTRY(drv, grp, BRL_KEY_NAME(drv, grp, key), nam)

static inline void setBrailleKeyTable(BrailleDisplay *brl,
...
extern void reverseTranslationTable(const TranslationTable from,
                                    TranslationTable to);

extern void setOutputTable(const TranslationTable table);
extern void makeOutputTable(const DotsTable dots);
extern void *translateOutputCells(unsigned char *target,
...
extern void *translateInputCells(unsigned char *target,
                                 const unsigned char *source, size_t count);
...
extern int connectBrailleResource(BrailleDisplay *brl, const char *identifier,
                                  const GioDescriptor *descriptor,
                                  BrailleSessionInitializer *initializeSession);

typedef int BrailleSessionEnder(BrailleDisplay *brl);

extern void disconnectBrailleResource(BrailleDisplay *brl,
...
typedef BraillePacketVerifierResult BraillePacketVerifier(
    BrailleDisplay *brl, const unsigned char *bytes, size_t size,
    size_t *length, void *data);

extern size_t readBraillePacket(BrailleDisplay *brl, GioEndpoint *endpoint,
                                void *packet, size_t size,
                                BraillePacketVerifier *verifyPacket,
                                void *data);

extern int writeBraillePacket(BrailleDisplay *brl, GioEndpoint *endpoint,
                              const void *packet, size_t size);

extern int writeBrailleMessage(BrailleDisplay *brl, GioEndpoint *endpoint,
                               int type, const void *packet, size_t size);

extern int acknowledgeBrailleMessage(BrailleDisplay *brl);

typedef int BrailleRequestWriter(BrailleDisplay *brl);

typedef size_t BraillePacketReader(BrailleDisplay *brl, void *packet,
                                   size_t size);
...
typedef BrailleResponseResult BrailleResponseHandler(BrailleDisplay *brl,
                                                     const void *packet,
                                                     size_t size);

extern int probeBrailleDisplay(BrailleDisplay *brl, unsigned int retryLimit,
                               GioEndpoint *endpoint, int inputTimeout,
                               BrailleRequestWriter *writeRequest,
                               BraillePacketReader *readPacket,
                               void *responsePacket, size_t responseSize,
                               BrailleResponseHandler *handleResponse);

typedef uint32_t KeyNumberSet;
#define KEY_NUMBER_BIT(number) (UINT32_C(1) << (number))

extern KeyNumberSet makeKeyNumberSet(KEY_NAME_TABLES_REFERENCE keys,
                                     KeyGroup group);

extern int enqueueKeyEvent(BrailleDisplay *brl, KeyGroup group,
                           KeyNumber number, int press);

extern int enqueueKeyEvents(BrailleDisplay *brl, KeyNumberSet set,
                            KeyGroup group, KeyNumber number, int press);

extern int enqueueKey(BrailleDisplay *brl, KeyGroup group, KeyNumber number);

extern int enqueueKeys(BrailleDisplay *brl, KeyNumberSet set, KeyGroup group,
                       KeyNumber number);

extern int enqueueUpdatedKeys(BrailleDisplay *brl, KeyNumberSet new,
                              KeyNumberSet *old, KeyGroup group,
                              KeyNumber number);

extern int enqueueUpdatedKeyGroup(BrailleDisplay *brl, unsigned int count,
                                  const unsigned char *new, unsigned char *old,
                                  KeyGroup group);

extern int enqueueXtScanCode(BrailleDisplay *brl, unsigned char code,
                             unsigned char escape, KeyGroup group00,
                             KeyGroup groupE0, KeyGroup groupE1);
