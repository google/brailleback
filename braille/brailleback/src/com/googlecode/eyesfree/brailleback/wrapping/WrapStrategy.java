...
/**
 * Handles the presentation of braille content that doesn't completely fit on
 * the braille display.
 */
public abstract class WrapStrategy {

    /**
     * Indicates a position in braille that marks an acceptable place to start
     * a new line, while indicating that the cell at the given position should
     * not be removed. For example, you can mark the first braille position
     * after a hyphen as an unremovable break point, since you wouldn't want it
     * to be truncated.
     *
     * <p>
     * <em>Note:</em> In the current line-breaking algorithm, there is an
     * implicit unremovable break point placed on the first braille position
     * after a contiguous string of removable break points (i.e. it is
     * acceptable to break right before the first character after a string of
     * removable break points in order to elide the removable break points). It
     * is acceptable, but not necessary, to explicitly mark the first braille
     * position after a string of removable break points as an unremovable
     * break point.
     * </p>
     */
    ...
    /**
     * Indicates a position in braille that marks an acceptable place to start
     * a new line, while indicating that the cell at the given position can be
     * removed if it occurs at the beginning or end of a line. For example, you
     * can mark whitespace as a removable break point because it is acceptable
     * to elide whitespace in most contexts.
     *
     * <p>
     * <em>Note:</em> The definition above implies that a line break might also
     * be placed <em>after</em> a removable break point, e.g. in this case with
     * a display width of 12 cells:
     * <pre>
     ...
     *   Display 2/2:             [picked%%%%%%]
     * </pre>
     * In this case, the line break will occur after the second removable break
     * point and before the word "picked" in order to remove the space from
    ...
    /**
     * Used in {@link #mSplitPoints} to indicate a position in braille where
     * a forced line break must occur, e.g. due to a newline or paragraph break.
     */
    protected static final int SPLIT_POINT = 1;

    /**
     * Used in the computed array {@link #mLineBreaks} to indicate the positions
     * in braille where lines begin.
     */
     ...
     * are not used and currently set to {@link #SPLIT_POINT}.
     */
    protected SparseIntArray mSplitPoints = new SparseIntArray();

    /**
     * An array where the keys are positions in braille at which we prefer
     * new lines to begin. The values are either
     * {@link #UNREMOVABLE_BREAK_POINT} or {@link #REMOVABLE_BREAK_POINT}.
     */
    protected SparseIntArray mBreakPoints = new SparseIntArray();

    /**
     * The precomputed line breaks that are used during panning. Positions
     * marked with {@link #LINE_BREAK} mark the beginning of a line; thus the
     * line break occurs right before such positions. The offset from one line
     * break to another is not necessarily less than or equal to the width of
     * the display; the line breaking algorithm may arrange the line breaks such
     * that removable break points are truncated at the end of a line.
     */
    private SparseIntArray mLineBreaks = new SparseIntArray();

    /**
     * Sets the current content context for the wrap strategy to calculate
     * panning positions. If any of these parameters change, you must call
     * {@link #setContent} again; otherwise, the panning calculations done in
     * methods like {@link #panTo} or {@link #panLeft} will be incorrect.
    ...
    public void setContent(Content content,
            ...
            int displayWidth) {
        if (content == null || translation == null || displayWidth <= 0
    ...
    protected void calculateSplitPoints() {
        ...
        for (int i = 0; i < text.length() - 1; ++i) {
            if (text.charAt(i) == '\n') {
                int cell = (i + 1 < textToCell.length)
                        ? textToCell[i + 1] : numCells;
    ...
    /**
     * Calculates the line breaks for the current content using the current
     * pivot point. The line breaks will occur such that the pivot point is
     * guaranteed to be at the beginning of some line.
     */
     ...
     * @param fix If {@code true}, then the display will pan to {@code position}
     *            as if the user had manually panned there. If {@code false},
     *            then the display will pan such that {@code position} becomes
     *            the leftmost cell on the display.
     */
    ...
    /**
     * Moves the display starting and ending positions to the left of the
     * current content.
     * @return {@code true} if the display was panned, or {@code false} if it's
     * at the left edge.
     */
    ...
    /**
     * Moves the display starting and ending positions to the right of the
     * current content.
     * @return {@code true} if the display was panned, or {@code false} if
    ...
    private int calculateDisplayStart(int end) {
        ...
        while (end > 0
            && mBreakPoints.get(end - 1) == REMOVABLE_BREAK_POINT) {
            end--;
        }
